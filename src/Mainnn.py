...
class AbstractClass:
    ...
    def is_internet_connected(self):
        try:
            ...
            messagebox.showinfo("Info", "Perang terhubung ke internet.")
            return True
        except OSError:
...
class MainFrame(tk.Frame, AbstractClass):
    ...
    def visualize(self, parent, ticker: str, data: pd.DataFrame):
        # 1. Buat object figure dari Matplotlib
        ...
        # 4. Gambar grafik harga penutup saham
        plot.clear()
        plot.plot(data["Close"])
        plot.set_title(f"Stock Price for {ticker}")
    ...
    def search(self):
        ...
        """
        q = self.entrybox.get().upper()

        if len(q) == 0:
            messagebox.showwarning(
                "Warning", "Kolom pencarian tidak boleh kosong")

        else:
            df = self.call_dataframe(ticker_symbol=q)
            self.visualize(parent=self.tradingCanvas, ticker=q, data=df)
